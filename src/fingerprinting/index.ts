...
import { generateSessionSeed, generateDomainSeed, pickResolution } from './noise';
...
async function injectFingerprintProtection(tabId: number, url: string): Promise<void> {
  const settings = await getFingerprintSettings();
...
function buildInjectionConfig(settings: FingerprintSettings, domain: string): InjectedScriptConfig {
  if (sessionSeed === null) {
    sessionSeed = generateSessionSeed();
  }
...
export async function handleFingerprintMessage(type: string, payload: unknown): Promise<unknown> {
  switch (type) {
