...
export async function updateLastAccessed(origin: string, chainType: DAppChainType): Promise<void> {
  const permission = await getPermission(origin, chainType);
...
export async function revokePermission(origin: string, chainType?: DAppChainType): Promise<void> {
  const store = await getStore();
...
export async function hasPermission(origin: string, chainType: DAppChainType): Promise<boolean> {
  const permission = await getPermission(origin, chainType);
...
export async function isMaxSitesReached(): Promise<boolean> {
  ...
  const origins = new Set(Object.values(store.permissions).map((p) => p.origin));
