  ...
  emit(event: string, ...args: unknown[]): boolean {
    ...
    eventListeners.forEach((listener) => {
      try {
        ...
        // Listener errors should not prevent other listeners from running.
      }
    });
    return true;
  }
...
class AintivirusEVMProvider extends SimpleEventEmitter {
    ...
    if (typeof methodOrPayload === 'string') {
      if (typeof paramsOrCallback === 'function') {
        this.request({ method: methodOrPayload })
          .then((result) =>
            (paramsOrCallback as (error: Error | null, result?: unknown) => void)(null, { result }),
          )
          .catch((error) =>
            (paramsOrCallback as (error: Error | null, result?: unknown) => void)(error),
          );
        return;
      }
  ...
  sendAsync(
    ...
    callback: (
      error: Error | null,
      result?: { id?: number; jsonrpc: string; result?: unknown; error?: unknown },
    ) => void,
  ): void {
