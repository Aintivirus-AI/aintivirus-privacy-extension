...
function matchEVMToken(token: EVMTokenBalance, query: string): TokenSearchMatch | undefined {
  ...
  if (addressMatch) {
    return {
      matchField: 'address',
      matchStart: addressMatch.start,
      matchLength: addressMatch.length,
    };
  }
...
export function filterNativeToken(token: NativeToken, query: string): NativeTokenWithMatch | null {
  const trimmedQuery = query.trim();
