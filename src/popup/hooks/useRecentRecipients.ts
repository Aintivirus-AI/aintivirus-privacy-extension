...
export function upsertRecipient(
  ...
  const existingIndex = recipients.findIndex((r) => r.address.toLowerCase() === normalizedAddress);
...
export function filterRecipients(recipients: RecentRecipient[], query: string): RecentRecipient[] {
  if (query.length < 2) {
  ...
  const updateLabel = useCallback(
    async (address: string, label: string) => {
      ...
      const updatedChainRecipients = currentChainRecipients.map((r) =>
        r.address.toLowerCase() === normalizedAddress ? { ...r, label } : r,
      );
