...
import { getCurrentConnection, getRecentBlockhash, getTransactionExplorerUrl } from './rpc';
...
export async function createTransferTransaction(
  ...
  if (!validateRecipient(recipient)) {
    throw new WalletError(WalletErrorCode.INVALID_RECIPIENT, 'Invalid recipient address');
  }
...
export async function sendSol(params: SendTransactionParams): Promise<SendTransactionResult> {
  const { recipient, amountSol } = params;
...
export async function confirmTransaction(
  ...
  try {
    ...
    return {
      confirmed: false,
      error:
        'Transaction confirmation timeout. The transaction may still succeed - check your wallet balance.',
    };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    ...
    if (errorMessage.includes('block height exceeded')) {
      ...
      return {
        confirmed: false,
        error:
          'Could not confirm transaction, but it may have succeeded. Check your wallet balance.',
      };
    }
...
export function wouldLeaveDust(currentBalance: number, sendAmount: number, fee: number): boolean {
...
export async function sendSPLToken(params: SendSPLTokenParams): Promise<SendTransactionResult> {
  ...
  try {
    ...
    transaction.add(
      createTransferInstruction(senderATA, recipientATA, keypair.publicKey, rawAmount),
    );
