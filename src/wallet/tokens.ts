...
const POPULAR_ERC20_TOKENS: PopularToken[] = [
  ...
  { mint: '0x6B175474E89094C44Da98b954EedeC4b3ddC6F00a', symbol: 'DAI', name: 'Dai Stablecoin', decimals: 18, logoUri: 'https://assets.coingecko.com/coins/images/9956/small/Badge_Dai.png', chainType: 'evm' },
...
function getCacheStatus(address: string, allowStale: boolean = false): { valid: boolean; stale: boolean } {
...
export async function getTokenBalances(
  ...
  if (!forceRefresh && cacheStatus.valid) {
    ...
    if (cacheStatus.stale && !backgroundRefreshInProgress) {
      ...
      tokenDedup.execute(bgKey, () => fetchTokenBalancesInternal(address), 0)
        .finally(() => {
...
async function fetchTokenBalancesInternal(address: string): Promise<SPLTokenBalance[]> {
  try {
    ...
    for (const token of tokens) {
      ...
      if (hiddenTokens.has(token.mint) && !customTokenMints.has(token.mint) && token.uiBalance > 0) {
    ...
    const visibleTokens = tokens.filter(t => 
      !hiddenTokens.has(t.mint) || customTokenMints.has(t.mint)
    );
    tokens.length = 0;
    ...
    for (const token of tokens) {
      const customToken = customTokens.find(ct => 
        ct.mint === token.mint && !ct.mint.startsWith('0x')
      );
      if (customToken) {
...
export async function removeCustomToken(mint: string): Promise<void> {
  ...
  const newHiddenTokens = alreadyHidden
    ? hiddenTokens 
    : [...hiddenTokens, normalizedMint];
