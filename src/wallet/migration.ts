...
import { getEVMAddressFromMnemonic, deriveAddressesForIndex } from './keychain';
import {
  generateSalt,
...
export async function detectVaultVersion(): Promise<VaultVersionInfo> {
  ...
  const vault = v2Result[STORAGE_KEYS.MULTI_WALLET_VAULT] as
    | (MultiWalletVault | MultiWalletVaultV3)
    | undefined;
...
export async function generateMasterVerifier(password: string, salt: string): Promise<string> {
...
export async function migrateV1ToV2(password: string): Promise<{
  ...
  if (versionInfo.version !== 1 || !versionInfo.legacyVault) {
    throw new WalletError(WalletErrorCode.MIGRATION_FAILED, 'No v1 vault found to migrate');
  }
...
export function migrateWalletToMultiChain(mnemonic: string, walletEntry: WalletEntry): WalletEntry {
...
export async function getMigrationStatus(): Promise<MigrationStatus> {
  ...
  if (versionInfo.version === 2 && versionInfo.multiWalletVault) {
    walletsNeedingEVM = versionInfo.multiWalletVault.wallets.filter((w) => !w.evmAddress).length;
  }
