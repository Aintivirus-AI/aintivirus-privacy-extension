...
export function historyKey(chain: string, address: string, limit: number, network?: string): string {
...
export async function batchRequests<K, V>(
  ...
  for (const key of keys) {
    ...
    const cached = await deduplicator.execute(
      `check:${cacheKey}`,
      async () => null,
      0
    );
  ...
  if (keysToFetch.length > 0) {
    ...
    const fetched = await deduplicator.execute(
      batchKey,
      () => batchFetcher(keysToFetch),
      cacheTtl
    );
