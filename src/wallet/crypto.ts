...
export async function encrypt(
  ...
  try {
    ...
    throw new WalletError(WalletErrorCode.ENCRYPTION_FAILED, 'Failed to encrypt data');
  }
}
...
export function getPasswordStrengthFeedback(password: string): {
  ...
  const strengthFactors = [isVeryLong, hasMultipleSpecial, hasMultipleNumbers].filter(
    Boolean,
  ).length;
