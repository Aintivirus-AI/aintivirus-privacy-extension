...
export async function addConnectionRecord(record: ConnectionRecord): Promise<void> {
...
export async function getLastConnectionForDomain(domain: string): Promise<ConnectionRecord | null> {
...
export async function getActiveConnection(domain: string): Promise<ActiveConnection | null> {
...
export async function getDomainSettings(domain: string): Promise<DomainSettings | null> {
...
export async function getDismissedWarningTimestamp(domain: string): Promise<number | null> {
  const result = await chrome.storage.local.get(STORAGE_KEYS.DISMISSED_WARNINGS);
  const dismissed: Record<string, number> = result[STORAGE_KEYS.DISMISSED_WARNINGS] || {};
