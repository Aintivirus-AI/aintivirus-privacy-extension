// Remote registry fetcher keeps Solana program risk data up to date (with bootstrap fallback).
...
const BOOTSTRAP_PROGRAMS: RemoteProgramInfo[] = [
  ...
  },
  {
    ...
    name: 'Compute Budget',
    description: 'Compute unit management',
    riskLevel: 'verified',
    category: 'system',
  },
  {
...
function customSettingToProgramInfo(programId: string, setting: CustomProgramSetting): ProgramInfo {
  return {
    programId,
...
function stringToRiskLevel(level: string): ProgramRiskLevel {
  switch (level) {
    ...
    case 'flagged':
      return ProgramRiskLevel.FLAGGED;
    case 'malicious':
...
export async function searchPrograms(query: string): Promise<ProgramInfo[]> {
  ...
  return registry.programs
    .filter((p) => p.name.toLowerCase().includes(lowerQuery) || p.programId.includes(query))
...
export async function getProgramsByRiskLevel(riskLevel: ProgramRiskLevel): Promise<ProgramInfo[]> {
  ...
  return registry.programs.filter((p) => p.riskLevel === levelString).map(remoteToProgramInfo);
}
