(function () {
  ...
  function serializeTransaction(transaction: unknown): string {
    ...
    if (
      typeof (transaction as { serializeMessage?: () => Uint8Array }).serializeMessage ===
      'function'
    ) {
  ...
  function handleWalletResponse(payload: {
    id: string;
    success: boolean;
    result?: unknown;
    error?: string;
  ...
  function wrapWallet(wallet: Record<string, unknown>): Record<string, unknown> {
    ...
    for (const key in wallet) {
      ...
      if (methodsToWrap.includes(key) && typeof value === 'function') {
        wrappedWallet[key] = createMethodWrapper(value.bind(wallet), key, wallet);
  ...
  function wrapMethodsInPlace(wallet: Record<string, unknown>): void {
    ...
    for (const methodName of methodsToWrap) {
      ...
      if (
        ...
        !(original as unknown as Record<string, boolean>)._aintivirus_wrapped
      ) {
        const wrapped = createMethodWrapper(original.bind(wallet), methodName, wallet);
