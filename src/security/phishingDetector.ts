// Domain risk evaluator combining threat intel, heuristics, and user overrides.
import { PhishingAnalysis, PhishingSignal, PhishingSignalType, RiskLevel } from './types';
...
// Main entry to evaluate a domain; returns structured phishing analysis for UI/actions.
export async function analyzeDomain(domain: string): Promise<PhishingAnalysis> {
  ...
  const isPhishing = signals.some(
    (s) => s.type === 'known_scam' || (s.type === 'homoglyph' && s.severity === 'high'),
  );
...
export async function shouldShowWarning(domain: string): Promise<boolean> {
  ...
  if (
    (await isSuspiciousTld(normalized)) &&
    containsSolanaKeywords(baseName, threatData.solanaKeywords)
  ) {
    return true;
  }
...
function checkTyposquatting(domain: string, legitimateDomains: string[]): string | null {
...
function findSimilarKnownDomain(domain: string, legitimateDomains: string[]): string | null {
  const baseDomain = domain.split('.')[0];
