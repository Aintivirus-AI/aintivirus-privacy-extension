...
function isProtectedSite(): boolean {
  ...
  return PROTECTED_SITES.some((site) => hostname === site || hostname.endsWith(`.${site}`));
}
...
function removeAdPlaceholders(): void {
  ...
  const advertisementContainers = document.querySelectorAll(
...
function showPhishingOverlay(
  ...
  const disclaimer = createTextElement(
    'p',
...
async function showConnectionWarning(
  ...
  return new Promise((resolve) => {
    ...
    const riskP = createTextElement(
      'p',
      `Risk Level: ${analysis.riskLevel.toUpperCase()}`,
      `av-risk av-risk-${analysis.riskLevel}`,
    );
    ...
    if (analysis.isPhishing) {
      modal.appendChild(
        createTextElement(
          'p',
          'This domain has been flagged as a potential phishing site.',
          'av-phishing-alert',
        ),
      );
    }
    ...
    modal.appendChild(
      createTextElement(
        'p',
        'AINTIVIRUS cannot guarantee safety. Verify this is the correct site.',
        'av-disclaimer',
      ),
    );
...
async function showTransactionWarning(
  ...
  return new Promise((resolve) => {
    ...
    const riskP = createTextElement(
      'p',
      `Risk Level: ${highestRisk.toUpperCase()}`,
      `av-risk av-risk-${highestRisk}`,
    );
    modal.appendChild(riskP);
