...
export async function getHeaderRuleStatus(): Promise<{
  ...
  const existingRules = await chrome.declarativeNetRequest.getDynamicRules();
  const headerRules = existingRules.filter((r) => r.id >= HEADER_RULE_START && r.id < 5000);

  return {
    refererStripping: headerRules.some(
      (r) => r.id === RULE_IDS.STRIP_REFERER || r.id === RULE_IDS.STRIP_REFERER_MAIN_FRAME,
    ),
    gpcEnabled: headerRules.some((r) => r.id === RULE_IDS.ADD_GPC),
    paramStripping: headerRules.some((r) => r.id >= RULE_IDS.STRIP_TRACKING_PARAMS_BASE),
