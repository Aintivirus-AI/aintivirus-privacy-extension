...
export function logBlockedRequest(tabId: number, url: string, ruleId: number): void {
...
export function logCookieCleanup(domain: string, count: number, mode: SitePrivacyMode): void {
...
export function getMetricsSummary(): {
  ...
  const blockedToday = metrics.recentBlocked.filter((r) => r.timestamp >= todayTimestamp).length;
...
export function getBlockedForDomain(domain: string): BlockedRequest[] {
  ...
  return metrics.recentBlocked.filter((r) => r.domain.toLowerCase() === normalized);
}
