  ...
  if (chrome.declarativeNetRequest.onRuleMatchedDebug) {
    ...
    chrome.declarativeNetRequest.onRuleMatchedDebug.addListener((info) => {
      ...
      if (tabId && tabId > 0) {
        ...
        logBlockedRequest(tabId, info.request.url, info.rule.ruleId);
  ...
  try {
    ...
    if (currentCount > previousCount) {
      ...
      for (let i = 0; i < newBlocks; i++) {
        ...
        if (ruleInfo) {
          logBlockedRequest(tabId, 'blocked-request', ruleInfo.rule.ruleId);
        } else {
          logBlockedRequest(tabId, 'blocked-request', 0);
        }
      }
    }
