...
export async function recordFetchError(url: string, error: string): Promise<void> {
...
export async function saveLastKnownGood(url: string, rules: string[]): Promise<void> {
  const domainRules = rules
    .filter((r) => r.startsWith('||'))
    .slice(0, MAX_LKG_RULES_PER_LIST * 0.7);
  const otherRules = rules
    .filter((r) => !r.startsWith('||'))
    .slice(0, MAX_LKG_RULES_PER_LIST * 0.3);
...
export async function getFilterListHealthSummary(): Promise<FilterListHealthSummary> {
  ...
  const lastRefresh = lists.length > 0 ? Math.max(...lists.map((l) => l.lastFetchAt)) : 0;
...
export async function trackUnsupportedPattern(url: string, pattern: string): Promise<void> {
